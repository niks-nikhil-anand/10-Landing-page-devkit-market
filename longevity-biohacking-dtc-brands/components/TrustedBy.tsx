...
const metrics = [
  { value: 350000, label: "Customers Served", prefix: "", suffix: "+", icon: Users },
  { value: 85, label: "Countries Covered", prefix: "", suffix: "", icon: Globe2 },
  { value: 4.9, label: "Customer Rating", prefix: "", suffix: "★", icon: Star, isFloat: true },
  { value: 18, label: "Products Delivered", prefix: "", suffix: "M+", icon: Package },
  { value: 95, label: "Customer Satisfaction", prefix: "", suffix: "%", icon: Heart },
];

const logos = [
  { name: "Forbes Wellness", subtitle: "MEDIA REPORT" },
  { name: "Men's Health", subtitle: "HEALTH HUB" },
  { name: "Women's Health", subtitle: "WELLNESS GUIDE" },
  { name: "TechCrunch", subtitle: "BIOTECH FEATURE" },
  { name: "Bloomberg Health", subtitle: "MARKETS REACTION" },
  { name: "Wellness Today", subtitle: "LIFESTYLE DAILY" },
];
...
function Counter({ value, prefix = "", suffix = "", isFloat = false }: { value: number; prefix?: string; suffix?: string; isFloat?: boolean }) {
        ...
        if (entries[0].isIntersecting && !hasAnimated) {
          ...
          const animate = (currentTime: number) => {
            ...
            if (isFloat) {
              ...
              setFloatCount(parseFloat(currentCount.toFixed(1)));
            } else {
  ...
  return (
    <span ref={elementRef}>
      {prefix}
      {isFloat ? floatCount.toFixed(1) : formatNumber(count)}
      {suffix}
    </span>
  );
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {metrics.map((metric, i) => (
            ...
            >
              ...
              <div className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                <Counter value={metric.value} prefix={metric.prefix} suffix={metric.suffix} isFloat={metric.isFloat} />
              </div>
        ...
        <div className="space-y-6 pt-4">
          <p className="text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            FEATURED IN EXPERT WELLNESS & TECHNOLOGY MEDIA
          </p>
